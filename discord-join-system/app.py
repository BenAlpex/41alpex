...
# ===== KONTROL =====
print("=" * 50)
print("🔍 Değişken Kontrolü:")
print(f"CLIENT_ID: {'✅ VAR' if CLIENT_ID else '❌ YOK'}")
print(f"CLIENT_SECRET: {'✅ VAR' if CLIENT_SECRET else '❌ YOK'}")
print(f"BOT_TOKEN: {'✅ VAR' if BOT_TOKEN else '❌ YOK'}")
print(f"REDIRECT_URI: {REDIRECT_URI}")
print("=" * 50)

if not CLIENT_ID or not CLIENT_SECRET or not BOT_TOKEN:
    print("❌ HATA: CLIENT_ID, CLIENT_SECRET veya BOT_TOKEN eksik!")
...
def delete_selected_users(user_ids):
    conn = sqlite3.connect('users.db')
    c = conn.cursor()
...
def admin_delete_selected():
    ...
    if user_ids:
        delete_selected_users(user_ids)
    return redirect('/admin')
